...
function getRookMoves(square: string, squares: Record<string, Piece>, color: 'w' | 'b'): string[] {
  const ff = FILES.indexOf(square[0]);
  const fr = RANKS.indexOf(square[1]);
  const valid: string[] = [];
...
function getPieceMoves(square: string, squares: Record<string, Piece>, color: 'w' | 'b', enPassant: string | null): string[] {
  ...
  if (p.type === 'r') return getRookMoves(square, squares, color);
  return [];
}
...
const START_FEN = 'r6r/pppppppp/8/8/8/8/PPPPPPPP/R6R w - - 0 1';
...
export default function RookPawnBoard({ onComplete, lessonId }: { onComplete: () => void; lessonId?: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full select-none" >
      ...
      <div className="text-center text-sm text-slate-600 max-w-sm px-4">
        ...
        <p className="text-xs text-slate-400 mt-1">Ладья ходит по прямым линиям на любое расстояние.</p>
      </div>
