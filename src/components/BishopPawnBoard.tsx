...
function getBishopMoves(square: string, squares: Record<string, Piece>, color: 'w' | 'b'): string[] {
  const ff = FILES.indexOf(square[0]);
  const fr = RANKS.indexOf(square[1]);
  const valid: string[] = [];
...
function getPieceMoves(square: string, squares: Record<string, Piece>, color: 'w' | 'b', enPassant: string | null): string[] {
  ...
  if (p.type === 'b') return getBishopMoves(square, squares, color);
  return [];
}
...
export default function BishopPawnBoard({ onComplete, lessonId }: { onComplete: () => void; lessonId?: string }) {
  const savedKey = lessonId ? `bishoppawn_progress_${lessonId}` : 'bishoppawn_progress';
