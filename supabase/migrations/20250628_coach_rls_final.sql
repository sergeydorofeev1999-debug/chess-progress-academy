...
CREATE POLICY "sel_enroll" ON course_enrollments FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "adm_enroll" ON course_enrollments FOR ALL USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
CREATE POLICY "coach_enroll" ON course_enrollments FOR SELECT USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'coach') AND EXISTS (SELECT 1 FROM courses WHERE courses.id = course_enrollments.course_id AND courses.coach_id = auth.uid()));
